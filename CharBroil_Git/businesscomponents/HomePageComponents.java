...
public class HomePageComponents extends ReusableLibrary {
	...
	public void validateSupportLink()	{
		try {
			...
			gc.clickItem(HomePage.linkSupport, "Home", "Support");
			Thread.sleep(3000);
			//gc.validateImage(System.getProperty("user.dir") + "\\images\\Support.png", "Support", "Support image");
			//gc.navigateToBack();
	...
	public void validateCharbroilLogo()	{
		try {			
			//gc.validateImage(System.getProperty("user.dir") + "\\images\\CbLogo.png", "Home", "Charbroil logo image");
	...
	public void searchItemFromHomePage()
	{
		try {
			...
			e.printStackTrace();
			report.updateTestLog("Exception in Searching product from search box homepage", "Exception is " + e, Status.FAIL);
		}
	}
		...
		try {
			...
			if(modelType.equalsIgnoreCase("valid"))	{
				if(wdu.objectExists(PartFinderPage.linkResultProducts))
					report.updateTestLog("Find A Part Result for valid product", "Results displayed against the searched model " + modelNo, Status.PASS);
				else
					report.updateTestLog("Find A Part Result for valid product", "Results not displayed against the searched model " + modelNo, Status.FAIL);
			}
	...
	public void validateFindMyModel()	{
		try		{
			...
			report.updateTestLog("Home Page", "Clicked on link for How to find the model number on your grill.", Status.DONE);
			...
			for(int count=1 ; count<=5 ; count++ )	{			
				if(driver.getCurrentUrl().contains("/find-my-model"))	{
					...
					report.updateTestLog("Home Page screen", "Not navigated on Find-my-model page in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
	...
	public void validateFooterContactSublinks()		{
		try	{
			...
			if(driver.findElement(HomePage.linkContactNo).getText().equalsIgnoreCase("1-866-239-6777"))
				report.updateTestLog("Home screen", "Contact no is showing correct at the homepage footer.", Status.PASS);
			else
				report.updateTestLog("Home screen", "Contact no is not showing correct at the homepage footer.", Status.FAIL);
	...
	public void validateFooterCustomerSupportSublinks()		{
		try	{
			...
			if(driver.findElement(HomePage.linkFormsInformationTitle).getText().equalsIgnoreCase("Forms & Information"))
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("support.charbroil.com"))	{
					...
					report.updateTestLog("Home Page screen", "Not navigated to Support Centers link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			...
			//gc.validateImage(System.getProperty("user.dir") + "\\images\\Parts.png", "Home", "Parts image");
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("charbroil.com/parts"))	{
					...
					report.updateTestLog("Home Page screen", "Not navigated to Parts link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			...
			e.printStackTrace();
			report.updateTestLog("Exception in validateFooterCustomerSupportSublinks", "Exception is " + e, Status.FAIL);
		}
	}
		...
		try	{			
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("charbroil.com/why-charbroil"))		{
					...
					if(driver.findElement(HomePage.linkAboutUsContentFirst).getText().equalsIgnoreCase("Innovating Since Day One"))
					...
					report.updateTestLog("Home screen", "Not navigated to About Us link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}			
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().equalsIgnoreCase("https://jobs.wcbradley.com/"))		{
					...
					report.updateTestLog("Home screen", "Not navigated to Careers link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			//driver.navigate().back();
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("www.sabergrills.com"))	{
					...
					report.updateTestLog("Home screen", "Not navigated to Saber Grills link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			//driver.navigate().back();
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("www.dancookusa.com"))	{
					...
					report.updateTestLog("Home screen", "Not navigated to Dancook Grills link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			//driver.navigate().back();
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("www.oklahomajoes.com"))		{
					...
					report.updateTestLog("Home screen", "Not navigated to Oklahoma link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			//driver.navigate().back();
	...
	public void validateFooterLinks()	{
		try		{
			...
			if(driver.findElement(HomePage.linkPageHeader).getText().equalsIgnoreCase("Privacy Policy"))
				...
				report.updateTestLog("Privacy policy screen", "Privacy policy header not showing correctly.", Status.FAIL);

			if(driver.getCurrentUrl().contains("/privacy-policy"))
				report.updateTestLog("Privacy policy screen", "Navigated to Privacy policy link correctly.", Status.PASS);
			else
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("/terms-of-use"))	{
					report.updateTestLog("Terms of Use screen", "Navigated to Terms of Use link correctly.", Status.PASS);
					break;
				}
				else if(count == 5)
					report.updateTestLog("Terms of Use screen", "Navigation to Terms of Use link failed.", Status.FAIL);
				else	{
					report.updateTestLog("Terms of Use screen", "Not navigated to Terms of Use link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			wdu.highlightElement(HomePage.linkPageHeader);
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("/supply-chains-act/"))	{
					report.updateTestLog("Supply chains act screen", "Navigated to Supply chains act link correctly.", Status.PASS);
					break;
				}
				else if(count == 5)
					report.updateTestLog("Supply chains act screen", "Navigation to Supply chains act link failed.", Status.FAIL);
				else	{
					report.updateTestLog("Supply chains act screen", "Not navigated to Supply chains act link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			wdu.highlightElement(HomePage.linkPageHeader);
			...
			if(driver.findElement(HomePage.linkPageHeader).getText().equalsIgnoreCase(text))
				report.updateTestLog("Supply chains act screen", "Supply chains act header showing correctly.", Status.PASS);
			else
				report.updateTestLog("Supply chains act screen", "Supply chains act header not showing correctly.", Status.FAIL);
	...
	public void validateWhatsHotPortion()		{
		try		{
			if(wdu.objectExists(HomePage.linkFeaturedWhatsHot))		{
				...
				gc.validateImage(System.getProperty("user.dir") + "\\images\\WhatsHot.png", "Home", "What's Hot image ");
			}	else
				report.updateTestLog("Home screen", "Featured link is not available inside What's Hot portion.", Status.FAIL);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void validateGrillForEveryYear()		{
		try		{
			if(wdu.objectExists(HomePage.linkGrillForEveryGriller))		{
				...
				gc.validateImage(System.getProperty("user.dir") + "\\images\\GrillForEveryGriller.png", "Home", "A Grill For Every Griller text ");
			}	else
	...
	public void validateGrillionYearsOfInnovation()		{
		try		{
			if(wdu.objectExists(HomePage.linkGrillionYearsOfInnovation))		{
				...
				gc.validateImage(System.getProperty("user.dir") + "\\images\\GrillionYearsOfInnovationImg1.png", "Home", "A Grillion Years Of Innovaton image ");
			}	else
	...
	public void validateNowYouCookinContent()	{
		try		{
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Holiday2017.png", "Home", "Holiday 2017 image ");
			driver.navigate().back();
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\TruInfrared.png", "Home", "TRU Infrared image ");
			driver.navigate().back();
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\HolidayRecipes.png", "Home", "Holiday Recipes image ");
			driver.navigate().back();
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\GrillCovers.png", "Home", "Grill Covers image ");
			driver.navigate().back();
	...
	public void validateWhatsHot()	{
		try		{
			wdu.waitUntilElementEnabled(HomePage.linkWhatsHotRedPlus1, 5);
			gc.scrollToElement(HomePage.linkWhatsHotRedPlus1);
			wdu.highlightElement(HomePage.linkWhatsHotRedPlus1);
			driver.findElement(HomePage.linkWhatsHotRedPlus1).click();
			gc.validateImage(System.getProperty("user.dir") + "\\images\\WhatsHotRedPlus1.png", "Home", "Whats Hot details image ");

			wdu.highlightElement(HomePage.linkWhatsHotRedPlus2);
			driver.findElement(HomePage.linkWhatsHotRedPlus2).click();
			gc.validateImage(System.getProperty("user.dir") + "\\images\\WhatsHotRedPlus2.png", "Home", "Whats Hot details image ");

			wdu.highlightElement(HomePage.linkWhatsHotRedPlus3);
			driver.findElement(HomePage.linkWhatsHotRedPlus3).click();
			gc.validateImage(System.getProperty("user.dir") + "\\images\\WhatsHotRedPlus3.png", "Home", "Whats Hot details image ");

			wdu.highlightElement(HomePage.linkWhatsHotRedPlus4);
			driver.findElement(HomePage.linkWhatsHotRedPlus4).click();
			gc.validateImage(System.getProperty("user.dir") + "\\images\\WhatsHotRedPlus4.png", "Home", "Whats Hot details image ");

			wdu.highlightElement(HomePage.linkCheckOutModularKitchen);
			driver.findElement(HomePage.linkCheckOutModularKitchen).click();
			//gc.validateImage(System.getProperty("user.dir") + "\\images\\BigEasyFryer.png", "Home", "Big Easy Fryer image ");
			if(driver.getCurrentUrl().contains("charbroil.com/modularoutdoorkitchen"))
				report.updateTestLog("Check out link in whats hot section", "Navigated to modular kitchen page.", Status.PASS);
			else
				report.updateTestLog("Check out link in whats hot section", "Not navigated to modular kitchen page.", Status.FAIL);
	...
	public void validateGrillForEveryGrillerButtons()	{
		try		{
			...
			report.updateTestLog("Home Page", "Clicked on CHARCOAL link in Grill for every griller section.", Status.DONE);
			...
			report.updateTestLog("Home Page", "Clicked on CHARCOAL shop button in Grill for every griller section.", Status.DONE);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Charcoal Grills.png", "Home", "Charcoal Grills image ");
			...
			report.updateTestLog("Home Page", "Clicked on ELECTRIC link in Grill for every griller section.", Status.DONE);
			...
			report.updateTestLog("Home Page", "Clicked on ELECTRIC shop button in Grill for every griller section.", Status.DONE);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Electric Grills.png", "Home", "Electric Grills image ");
			...
			report.updateTestLog("Home Page", "Clicked on GAS shop button in Grill for every griller section.", Status.DONE);
			...
			report.updateTestLog("Home Page", "Clicked on PORTABLE link in Grill for every griller section.", Status.DONE);
			...
			report.updateTestLog("Home Page", "Clicked on PORTABLE shop button in Grill for every griller section.", Status.DONE);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Portable Grills.png", "Home", "Portable Grills image ");
			...
			report.updateTestLog("Home Page", "Clicked on SMOKERS & FRYERS link in Grill for every griller section.", Status.DONE);
			...
			report.updateTestLog("Home Page", "Clicked on SMOKERS & FRYERS shop button in Grill for every griller section.", Status.DONE);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Smokers&Fryers.png", "Home", "Smokers & Fryers image ");
	...
	public void validateSocialMediaLinksFooter()	{
		try	{
			...
			e.printStackTrace();
			report.updateTestLog("Exception in validateSocialMediaLinksFooter", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
		...
		try	{
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("https://www.facebook.com/"))	{//charbroil
					report.updateTestLog("Home Page screen", "Navigated to facebook link - "+driver.getCurrentUrl(), Status.PASS);
					break;
				}
				else if(count==5)
					report.updateTestLog("Home Page screen", "Not navigated to facebook link.", Status.FAIL);
				else	{
					report.updateTestLog("Home Page screen", "Not navigated to facebook link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			driver.close();
			...
			e.printStackTrace();
			report.updateTestLog("Exception in footerFacebookValidation", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
		...
		try	{
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("https://www.pinterest.com/charbroil/"))	{
					report.updateTestLog("Home Page screen", "Navigated to Pinterest link - "+driver.getCurrentUrl(), Status.PASS);
					break;
				}
				else if(count==5)
					report.updateTestLog("Home Page screen", "Not navigated to Pinterest link.", Status.FAIL);
				else	{
					report.updateTestLog("Home Page screen", "Not navigated to Pinterest link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			driver.close();
			...
			e.printStackTrace();
			report.updateTestLog("Exception in footerPintrestValidation", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
		...
		try	{
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("https://www.instagram.com/"))	{//charbroilgrills/
					report.updateTestLog("Home Page screen", "Navigated to Instagram link - "+driver.getCurrentUrl(), Status.PASS);
					break;
				}
				else if(count==5)
					report.updateTestLog("Home Page screen", "Not navigated to Instagram link.", Status.FAIL);
				else	{
					report.updateTestLog("Home Page screen", "Not navigated to Instagram link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			driver.close();
			...
			e.printStackTrace();
			report.updateTestLog("Exception in footerInstagramValidation", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
		...
		try	{
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("https://www.youtube.com/"))	{//user/charbroil/featured
					report.updateTestLog("Home Page screen", "Navigated to YouTube link - "+driver.getCurrentUrl(), Status.PASS);
					break;
				}
				else if(count==5)
					report.updateTestLog("Home Page screen", "Not navigated to YouTube link.", Status.FAIL);
				else	{
					report.updateTestLog("Home Page screen", "Not navigated to YouTube link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			driver.close();
		...
		try	{
			...
			for(int count=1 ; count<=5 ; count++ )	{
				if(driver.getCurrentUrl().contains("https://twitter.com/charbroil"))	{
					report.updateTestLog("Home Page screen", "Navigated to twitter link - "+driver.getCurrentUrl(), Status.PASS);
					break;
				}
				else if(count==5)
					report.updateTestLog("Home Page screen", "Not navigated to twitter link.", Status.FAIL);
				else	{
					report.updateTestLog("Home Page screen", "Not navigated to twitter link in 2 seconds. Will retry in another 2 seconds.", Status.DONE);
					Thread.sleep(2000);
				}
			}
			driver.close();
	...
	public void validateSmokersFryersLandingPage()		{
		try		{
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Smokers&Fryers.png", "Smokers & Fryers", "Smokers & Fryers image");
			...
			wdu.waitUntilElementEnabled(HomePage.linkShopMenuTitle, 10);
			gc.scrollToElement(HomePage.linkShopMenuTitle);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Smokers&FryersMenu.png", "Shop", "Smokers & Fryers menu image");
	...
	public void validateAccessoriesLandingPage()		{
		try		{
			...
			gc.validateImage(System.getProperty("user.dir") + "\\images\\Accessories.png", "Accessories", "Accessories image");
			...
			wdu.waitUntilElementEnabled(HomePage.linkShopMenuTitle, 10);
			gc.scrollToElement(HomePage.linkShopMenuTitle);
			gc.validateImage(System.getProperty("user.dir") + "\\images\\AccessoriesMenu.png", "Shop", "Accessories menu image");
	...
	public void validateTrueInfra_URL(){
		try{
			...
			{
				report.updateTestLog("URL content Validation", "URL is containing the expected text: "+expectedText, Status.PASS);
			}
			...
			{
				report.updateTestLog("URL content Validation", "URL is not containing the expected text: "+expectedText, Status.FAIL);
			}

		}
		catch(Exception e){
			report.updateTestLog("URL content Validation", "Exception while validating URL Content. Exception is: "+e, Status.FAIL);
		}
	}	
	...
	public void checkFindAPartResults()	{
		try	{
			...
			if(expectedHeader.equalsIgnoreCase(actualHeader))
				report.updateTestLog("Find A Part result validation", "Results header is displayed as - "+actualHeader, Status.PASS);
			else
				report.updateTestLog("Find A Part result validation", "Results header is displayed as - "+actualHeader, Status.FAIL);

			//check the name of the product
			String prodName = driver.findElement(HomePage.lblPartFinderHeader).getText().split(":")[1].trim();
			report.updateTestLog("Find A Part result validation", "Product name is diaplyed as - "+prodName, Status.DONE);
			...
			for (int i=0 ; i<size ; i++) {
				...
				webElement.click();
				Thread.sleep(1000);
				report.updateTestLog("Find A Part result validation", "Clicked on Guide section link - "+text, Status.DONE);
	...
	public void validateContactUsEU()	{		
		try {
			...
			/*wdu.waitUntilElementLocated(HomePage.linkContactUsHeaderEU, 5);
				...
				report.updateTestLog("Contact us link validation", "Contact us header is present. Showing as - "+header, Status.PASS);*/
			...
			if(currUrl.contains(expectedUrl))
				report.updateTestLog("Contact us link validation", "Navigated to expected URL - "+currUrl, Status.PASS);
			else
				report.updateTestLog("Contact us link validation", "Not navigated to expected URL - "+currUrl, Status.FAIL);
		...
		try {
			...
			for (String handle : driver.getWindowHandles()) {
				if(!handle.equals(mainWindowHandle))	{
					...
					report.updateTestLog("Product registration link validation", "Product Registration label is present.", Status.PASS);
					driver.close();
				}
			}
			driver.switchTo().window(mainWindowHandle);
			...
			e.printStackTrace();
			report.updateTestLog("Exception in validateProductRegistration", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
				...
				if(!handle.equals(mainWindowHandle))	{
					...
					for(int i=0 ; i<5 ; i++)	{
						...
						if(currUrl.contains("charbroil.dk"))	{
							report.updateTestLog("Flag DANSK link validation", "Navigated to correct URL - "+currUrl, Status.PASS);
							break;
						}
						else if(i==4)
							report.updateTestLog("Flag DANSK link validation", "Not navigated to correct URL. Opened url - "+currUrl, Status.FAIL);
						else
							Thread.sleep(2000);
					}
					driver.close();
				...
				if(!handle.equals(mainWindowHandle))	{
					...
					for(int i=0 ; i<5 ; i++)	{
						...
						if(currUrl.contains("charbroil.de"))	{
							report.updateTestLog("Flag DEUTSCH link validation", "Navigated to correct URL - "+currUrl, Status.PASS);
							break;
						}
						else if(i==4)
							report.updateTestLog("Flag DEUTSCH link validation", "Not navigated to correct URL. Opened url - "+currUrl, Status.FAIL);
						else
							Thread.sleep(2000);
					}
					driver.close();
				...
				if(!handle.equals(mainWindowHandle))	{
					...
					for(int i=0 ; i<5 ; i++)	{
						...
						if(currUrl.contains("charbroil.fr"))	{
							report.updateTestLog("Flag FRANCAIS link validation", "Navigated to correct URL - "+currUrl, Status.PASS);
							break;
						}
						else if(i==4)
							report.updateTestLog("Flag FRANCAIS link validation", "Not navigated to correct URL. Opened url - "+currUrl, Status.FAIL);
						else
							Thread.sleep(2000);
					}
					driver.close();
				...
				if(!handle.equals(mainWindowHandle))	{
					...
					for(int i=0 ; i<5 ; i++)	{
						...
						if(currUrl.contains("charbroil.com"))	{
							report.updateTestLog("Flag USA link validation", "Navigated to correct URL - "+currUrl, Status.PASS);
							break;
						}
						else if(i==4)
							report.updateTestLog("Flag USA link validation", "Not navigated to correct URL. Opened url - "+currUrl, Status.FAIL);
						else
							Thread.sleep(2000);
					}
					driver.close();
	...
	public void validateProductsLinkEU()	{
		try {
			...
			//gc.clickItem(HomePage.linkProductsForCountries, "Home", driver.findElement(HomePage.linkProductsForCountries).getText());
			...
			currUrl = driver.getCurrentUrl();
			if(currUrl.contains(expectedUrl))
				report.updateTestLog("Products link validation", "Navigated to expected url. Current url is - "+currUrl, Status.PASS);
			else
				report.updateTestLog("Products link validation", "Not navigated to expected url. Current url is - "+currUrl, Status.FAIL);
	...
	public void validateRecipesLinkEU()	{
		try {
			...
			gc.clickItem(HomePage.linkRecipesForCountries, "Home", driver.findElement(HomePage.linkRecipesForCountries).getText());
			Thread.sleep(2000);

			currUrl = driver.getCurrentUrl();
			if(currUrl.contains(expectedUrl))
				report.updateTestLog("Recipes link validation", "Navigated to expected url. Current url is - "+currUrl, Status.PASS);
			else
				report.updateTestLog("Recipes link validation", "Not navigated to expected url. Current url is - "+currUrl, Status.FAIL);
	...
	public void validateTipsLinkEU()	{
		try {
			...
			gc.clickItem(HomePage.linkTipsForCountries, "Home", driver.findElement(HomePage.linkTipsForCountries).getText());
			Thread.sleep(2000);

			currUrl = driver.getCurrentUrl();
			if(currUrl.contains(expectedUrl))
				report.updateTestLog("Tips link validation", "Navigated to expected url. Current url is - "+currUrl, Status.PASS);
			else
				report.updateTestLog("Tips link validation", "Not navigated to expected url. Current url is - "+currUrl, Status.FAIL);
	...
	public void validateSupportLinkEU()	{
		try {
			...
			report.updateTestLog("Home Page", "Support link is present.", Status.PASS);
			gc.clickItem(HomePage.linkSupportForCountries, "Home", driver.findElement(HomePage.linkSupportForCountries).getText());
			Thread.sleep(2000);

			currUrl = driver.getCurrentUrl();
			if(currUrl.contains(expectedUrl))
				report.updateTestLog("Support link validation", "Navigated to expected url. Current url is - "+currUrl, Status.PASS);
			else
				report.updateTestLog("Support link validation", "Not navigated to expected url. Current url is - "+currUrl, Status.FAIL);
	...
	public void searchFromHomeEU()
	{
		try {
			...
			e.printStackTrace();
			report.updateTestLog("Exception in Searching product from search box homepage", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateSearchResultsTabEU()		{
		try		{			
			String searchResult = "", expectedProductTab = "", expectedRecipesTab = "", expectedTipsTab = "", expectedSupportTab = "";
			...
			//Verify Product tab
			if(driver.findElement(ProductListingPage.linkResultsProductsTabEU).getText().equalsIgnoreCase(expectedProductTab))
				report.updateTestLog("Search Results Screen", "Products tab is displayed.", Status.PASS);
			else
				report.updateTestLog("Search Results Screen", "Products tab is not displayed.", Status.FAIL);

			searchResult = driver.findElement(ProductListingPage.lblPageHeaderTitle).getText();			
			report.updateTestLog("Search result screen validation", "Displaying search result as - "+searchResult, Status.PASS);
			...
			Thread.sleep(1000);
			if(driver.findElement(ProductListingPage.linkResultsRecipesTabEU).getText().equalsIgnoreCase(expectedRecipesTab))
				report.updateTestLog("Search Results Screen", "Recipes tab is displayed.", Status.PASS);
			else
				report.updateTestLog("Search Results Screen", "Recipes tab is not displayed.", Status.FAIL);
			...
			Thread.sleep(1000);
			if(driver.findElement(ProductListingPage.linkResultsHowToTabEU).getText().equalsIgnoreCase(expectedTipsTab))
				report.updateTestLog("Search Results Screen", "How To tab is displayed.", Status.PASS);
			else
				report.updateTestLog("Search Results Screen", "How To tab is not displayed.", Status.FAIL);
			...
			Thread.sleep(1000);
			if(driver.findElement(ProductListingPage.linkResultsSupportTabEU).getText().equalsIgnoreCase(expectedSupportTab))
				report.updateTestLog("Search Results Screen", "Support tab is displayed.", Status.PASS);
			else
				report.updateTestLog("Search Results Screen", "Support tab is not displayed.", Status.FAIL);
	...
	public void validateInvalidProdSearchEU()	{
		try	{
			...
			if(result.contains("0 RESULTS FOR "))
				report.updateTestLog("Validate invalid search result", "Invalid search result is displayed as - "+result.trim(), Status.PASS);
			else
				report.updateTestLog("Validate invalid search result", "Expected result to contain - '0 RESULTS FOR ' text. "
						+ "Invalid search result is displayed as - "+result.trim(), Status.FAIL);
		} catch (Exception e) {
			e.printStackTrace();
			report.updateTestLog("Exception in validateInvalidProdSearchEU", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateExploreGrillButton()	{
		try	{
			...
			Thread.sleep(1000);
			report.updateTestLog("Whats Hot section validation", "Clicked on Explore Our Hydrid Grill button.", Status.DONE);
			...
			e.printStackTrace();
			report.updateTestLog("Exception in validateExploreGrillButton", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateBreadcrumbEU(String prodCatg)	{
		try	{
			...
			if(breadcrumbtext.toUpperCase().contains(prodCatg.toUpperCase()))
				report.updateTestLog(prodCatg+" products page validation", "Breadcrumb is displayed as - "+breadcrumbtext, Status.PASS);
			else
				report.updateTestLog(prodCatg+" products page validation", "Breadcrumb is displayed as - "+breadcrumbtext
						+". Expected to contain text - "+prodCatg, Status.FAIL);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void validateGrillForEveryGrillerEU()	{
		try	{
			...
			btnText = ele.getText();
			ele.click();
			report.updateTestLog("Grills for every griller section validation", "Clicked on Charcoal Grills button.", Status.PASS);
			...
			report.updateTestLog("Grills for every griller section validation", "Clicked on Shop All Charcoal Grills button.", Status.DONE);
			...
			btnText = ele.getText();
			ele.click();
			report.updateTestLog("Grills for every griller section validation", "Clicked on Electric Grills button.", Status.PASS);
			...
			report.updateTestLog("Grills for every griller section validation", "Clicked on Shop All Electric Grills button.", Status.DONE);
			validateBreadcrumbEU(btnText);
			driver.navigate().back();
			driver.navigate().back();
			...
			btnText = ele.getText();
			ele.click();
			report.updateTestLog("Grills for every griller section validation", "Clicked on Gas Grills button.", Status.PASS);
			...
			report.updateTestLog("Grills for every griller section validation", "Clicked on Shop All Gas Grills button.", Status.DONE);
			validateBreadcrumbEU(btnText);
			driver.navigate().back();
			driver.navigate().back();
			...
			btnText = ele.getText();
			ele.click();
			report.updateTestLog("Grills for every griller section validation", "Clicked on Portable Grills button.", Status.PASS);
			...
			report.updateTestLog("Grills for every griller section validation", "Clicked on Shop All Portable Grills button.", Status.DONE);
			validateBreadcrumbEU(btnText);
			driver.navigate().back();
			driver.navigate().back();
			...
			btnText = ele.getText();
			ele.click();
			report.updateTestLog("Grills for every griller section validation", "Clicked on Smokers Grills button.", Status.PASS);
			...
			report.updateTestLog("Grills for every griller section validation", "Clicked on Shop All Smokers Grills button.", Status.DONE);
			validateBreadcrumbEU(btnText);


		} catch (Exception e) {
			e.printStackTrace();
			report.updateTestLog("Exception in validateGrillForEveryGriller", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateContactFooterEU()	{
		try	{
			...
			if(driver.findElement(HomePage.linkFooterContactSendEmailEU).getText().equalsIgnoreCase(expectedSendEmailText))
			...
			report.updateTestLog("Footer contact validation", "Clicked on imprint option in footer contacr section.", Status.DONE);
			Thread.sleep(1000);
	...
	public void validateCustomerSupportFooterEU()	{
		try	{
			...
			report.updateTestLog("Footer customer support validation", "Clicked on product help option in footer support section.", Status.DONE);			
			Thread.sleep(1000);
			...
			if(driver.findElement(HomePage.lblPageTitleEU).isDisplayed())
				report.updateTestLog("Footer customer support validation", "Product help header is displayed as - "+
						actualHeader, Status.PASS);
			else
				report.updateTestLog("Footer customer support validation", "Product header is not displayed as expected. "
						+ "Header displayed as - "+actualHeader, Status.FAIL);

			driver.navigate().back();

		} catch (Exception e) {
			e.printStackTrace();
			report.updateTestLog("Exception in validateCustomerSupportFooterEU", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateCompanyFooterEU()	{
		try	{
			...
			report.updateTestLog("Footer company validation", "Clicked on Dancook Grills option in footer support section.", Status.DONE);			
			Thread.sleep(1000);
			...
			if(actualUrl.contains("www.dancook.dk/"))
				...
				report.updateTestLog("Footer company validation", "Not navigated to dancook grills page. "
						+ "Landed to following url - "+actualUrl, Status.FAIL);
			...
			report.updateTestLog("Footer customer support validation", "Clicked on Magazine option in footer support section.", Status.DONE);			
			Thread.sleep(2000);
			...
			for (String handle : driver.getWindowHandles()) {
				if(!handle.equals(mainWindowHandle))	{
					...
					if(driver.getCurrentUrl().contains("pdf"))
						report.updateTestLog("Footer customer support validation", "Magazine opened pdf file in new tab.", Status.PASS);
					else
						report.updateTestLog("Footer customer support validation", "Magazine not opened a pdf file in new tab.", Status.FAIL);
					driver.close();
				}
			}
	...
	public void validateDataAccessRewustFooterEurope()	{
		try	{
			...
			Thread.sleep(2000);
			report.updateTestLog("Data Subject Access link validation in footer", "Clicked on data subject access request link.", Status.DONE);
			String mainWindow = driver.getWindowHandle();
			driver.switchTo().window(new ArrayList<String>(driver.getWindowHandles()).get(1));
			...
			for(int i=0 ; i<5 ; i++)	{
				if(url.contains("app.onetrust.com"))	{
					report.updateTestLog("Data Subject Access link validation in footer", "Navigated to expected url - "
							+url, Status.PASS);
					break;
				}
				else if(i==4)
					report.updateTestLog("Data Subject Access link validation in footer", "Not navigated to expected url. Expected url to "
							+ "contain 'app.onetrust.com'. Actual url - "+url, Status.FAIL);
				else
					Thread.sleep(2000);
			}
			...
			e.printStackTrace();
			report.updateTestLog("Exception in validateDataAccessRewustFooterEurope", "Exception is " + e.getMessage(), Status.FAIL);
		}
	}
	...
	public void validateFindAPartPage()		{
		...
		try {
			if(driver.findElement(HomePage.lstSortBy).isDisplayed())
			{
				...
				Thread.sleep(2000);
				report.updateTestLog("Find A Part Page", sortvalue+" value selected from the drop down menu", Status.DONE);
				if(driver.findElement(HomePage.lnkAddToCart).isDisplayed())
				{
					...
					{
						report.updateTestLog("Find A Part page", "Guide and Manual section link is displayed as expected", Status.PASS);	
						driver.findElement(HomePage.lnkGuidesSection).click();
						String url=driver.getCurrentUrl().toLowerCase();
						if(url.contains("manual"))
							report.updateTestLog("Guide and manual page", "Navigation to product manual page is successful", Status.PASS);
						else
							report.updateTestLog("Guide and manual page", "Navigation to product manual page is not successful", Status.FAIL);
					}
					else
					{
						report.updateTestLog("Find A Part page", "Guide and Manual section link is not displayed as expected", Status.FAIL);	
					}
